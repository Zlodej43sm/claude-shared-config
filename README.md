...
ready-made skills for Jira/Confluence context-gathering, Bitbucket PR review and
comment-answering, SonarQube triage, PR-splitting, and plan-first feature
implementation.
...
the per-project values. Safe to re-run; it refuses to clobber a non-symlink.
...
| --- | --- | --- |
| `answer-pr-comments` | Composes and posts replies to unanswered human reviewer comments on a Bitbucket PR; delegates the fetch/research/compose/post cycle to the `comment-responder` agent. | `/answer-pr-comments <pr-number> [<reviewer-name>] [--dry-run]` |
...
| `pr-reviewer` | Reviews a Bitbucket PR end-to-end: fetches diff/commits/comments/Jira context, classifies changed files, reads the working tree, and returns one structured review. Read-only. | `pr-review` |
