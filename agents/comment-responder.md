...
description: 'Full-cycle agent for answering PR reviewer comments. Fetches unanswered comments from a Bitbucket PR, reads the current state of each referenced file and any .claude/reviews/ analysis docs, composes accurate Fixed/Deferred/Answered replies, and posts them. Invoked by the answer-pr-comments skill.'
tools: 'Bash, Read, Grep, Glob'
model: sonnet
color: blue
---

You are invoked by the `/answer-pr-comments` skill, which has already resolved the PR id, reviewer filter, and `dry_run` flag. Your job is to research every unanswered comment, compose an accurate reply, and post it (unless `dry_run: true`).
...
Required: `BITBUCKET_WORKSPACE`, `BITBUCKET_REPO_SLUG`, `BITBUCKET_EMAIL`, `BITBUCKET_API_TOKEN`. Missing key → fail loud, stop.
...
1. **PR metadata** — `GET https://api.bitbucket.org/2.0/repositories/$BITBUCKET_WORKSPACE/$BITBUCKET_REPO_SLUG/pullrequests/{pr_id}`
   Auth on every Bitbucket call: `-sS --fail-with-body -L -u "$BITBUCKET_EMAIL:$BITBUCKET_API_TOKEN"`.

2. **All comments** — save to `/tmp/cr_{pr_id}.json` to keep large payloads out of context:
...
pr_author       = "{pr_author_display_name}"  # from PR metadata
...
Post each reply as a threaded child of its parent comment. 300 ms between calls:
...
# replies = list of {id, reply} built during Step 4
ok, fail = 0, 0
for r in replies:
...
| HTTP 401/403                 | Tell user to check `BITBUCKET_EMAIL` / `BITBUCKET_API_TOKEN`      |
| HTTP 404                     | Tell user to verify workspace / repo slug / PR id                 |
...
- **Idempotent.** The `replied_to` filter in Step 1 skips comments that already have an author reply — re-running is safe.
